...
int generarNumeroAleatorio() {
    ...
    if (esPrimo(numero)) {
        // Hay una probabilidad del 50% de convertir el número en no primo
        if (rand() % 2 == 0) {
            numero++;
            if (numero > 100) {
                numero -= 2;
...
int main() {
    ...
    while (true) {
            ...
            if (evento.empty()) {
                if (lista.contarEventos("A") == 0) {
                    ...
                    if (compartidos || lista.contarEventos("C") > 0) {
                        compartidos = (rand() % 2 == 0); // 50% de probabilidad de compartir datos
                        if (!compartidos) {
        ...
        if (evento == "A" || evento == "B" || evento == "C") {
            ...
            if (evento == "C") {
                if (compartidos) {
                    ...
                    // Buscar otros eventos A y B para que el C comparta datos
                    while (true) {
        ...
        if (evento == "C" && compartidos) {
            break; // Rompemos el bucle si se compartieron los datos
        }
    }

    std::cout << "Total de eventos: " << lista.tamano << std::endl;

    // Mostrar todos los nodos al finalizar
